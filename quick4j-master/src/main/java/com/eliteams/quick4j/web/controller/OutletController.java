...
//        int flag=1;
    ...
    @ResponseBody
    public List<outlet> excelOut(HttpServletRequest request, final int[] ids ) throws FileNotFoundException, UnsupportedEncodingException {
        Map<String, Object> modelmap = new HashMap<>();
    ...
    public static String[] excelTitle() {
        String[] strArray = { "序号", "统计年份", "统计月份", "统计日","排污口编号","单位名称","排放口名称","排放口类型","所在市(city)","所在县(市/区)","所在乡镇","详细地址","排放口经度","排放口纬度","是否取得审批手续","审批单位及审批文号","批复主要污染物种类","批复日允许排放量(吨/日)","批复年允许排放量(吨/年)","是否发放许可证","排污许可证号","投入使用时间","是否为非法排污口","是否为设置不合理排污口","排放口靠河岸位置","污水处理工艺","废水排放方式",
